...
def prune_wanda(args, model, tokenizer, dataloader, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    layers = model.model.layers
    for i in tqdm(range(len(layers)), desc="Pruning layers", dynamic_ncols=True):
        layer = layers[i]
        subset = find_layers(layer)
...
@torch.no_grad()
def prune_sparsegpt(args, model, tokenizer, dataloader, dev, prune_n=0, prune_m=0):
...
@torch.no_grad()
def prune_ablate(args, model, tokenizer, dataloader, dev, prune_n=0, prune_m=0):
    ## SparseGPT code available at: https://github.com/IST-DASLab/sparsegpt/tree/f5c25005a61f96a0933ca2f95705a963585aafaa
    # print('Starting ...')
    # dataloader, _ = get_loaders("c4",nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer)
